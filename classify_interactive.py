...
dclass["Vton"] = df.spkt.apply(
    lambda x: 1 if len(x) > 3 and stim["delay"] <= x[-1] <= stimend + 5 else 0
)
...
for col, s in zip(subtypes, scalar):
    dclass[col] = dclass[col] * s
...
fig = plt.figure()
ax = fig.add_subplot()
for a, c in zip(set(df["amp"]), ("g", "r", "m")):
    dclass.loc[df.amp == a].plot.scatter(
        "cellnum", "Vsubth", marker="o", color=c, ax=ax
    )
for a, c in zip(set(df["amp"]), ("g", "r", "m")):
    dclass.loc[df.amp == a].plot.scatter("cellnum", "Vph", marker="x", color=c, ax=ax)
for a, c in zip(set(df["amp"]), ("g", "r", "m")):
    dclass.loc[df.amp == a].plot.scatter("cellnum", "Vton", marker="s", color=c, ax=ax)
for a, c in zip(set(df["amp"]), ("g", "r", "m")):
    dclass.loc[df.amp == a].plot.scatter(
        "cellnum", "Vton_brfps", marker="^", color=c, ax=ax
    )
for a, c in zip(set(df["amp"]), ("g", "r", "m")):
    dclass.loc[df.amp == a].plot.scatter(
        "cellnum", "Vton_susps", marker="*", color=c, ax=ax
    )
plt.ylim(0.1, 1.1)
plt.yticks([0.1, 1.1], "")
plt.ylabel("Subtypes")
...
dclass.plot(
    "cellnum", "Vsubth", kind="scatter", s=s, c="y", label="Subthreshold", ax=ax
)
...
dclass.plot(
    ...
    s=s,
    c="k",
    label="Tonic with sus post-stim",
    ax=ax,
)
dclass.plot(
    "cellnum",
    ...
    s=s,
    c="c",
    label="Tonic with brief post-stim",
    ax=ax,
)
...
fig, ax = plt.subplots()
ax.clear()
for a, c in zip(set(df["amp"]), ("g", "r")):
    dclass.loc[df.amp == a].plot.scatter("cellnum", "Vsubth", color=c, ax=ax)
